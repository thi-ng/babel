...
this file, so this step isn't necessary from the command line.
...
#+BEGIN_SRC clojure :tangle ../{{tangle-target}}src/cljx/{{ns-root-path}}/core.cljx :noweb yes :mkdirp yes :padline no
  (ns {{ns-root}}.core)
